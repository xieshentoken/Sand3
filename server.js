...
async function api(request, response, url) {
  ...
  if (request.method === "POST" && url.pathname === "/api/database/import-pdf2") {
    ...
    if (existing && url.searchParams.get("force") !== "1") { request.resume(); return sendJson(response, 200, { ...existing, skipped: true }); }
...
createServer(async (request, response) => {
  ...
  console.log(`Sand3 running at http://127.0.0.1:${port}`);
